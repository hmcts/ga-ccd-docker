...
> NOTE: If you want to avoid install gradle, you can run the tests from the docker container ([Section 6.1](#61-default-ccd-definition-setup---docker))

##### 6.1 default ccd definition setup - docker

_from the root of the ccd-definition-store-api repository:_

```bash
source ../ga-ccd-docker/bin/set-environment-variables.sh
export TEST_URL=http://localhost:4451
docker run -i -v "$(pwd)":/home/gradle/project -w /home/gradle/project --network=host --env-file ../ga-ccd-docker/bin/env_variables_all.txt gradle ./gradlew clean smoke
```
